...
public class Config {
    ...
    private void runConfigSetup(String modid, boolean createModConfigFolder, boolean hasCustomName, String customConfigName) {
        ...
        if (defaultConfigFile == null) {
            SimpleStorageSystems.LOGGER.log(Level.SEVERE, "Unable to find the defaults config.yml for mod " + modid + " in assets/" + modid);
            System.exit(-1);
            return;
        }

        if (!config.exists()) {
            SimpleStorageSystems.LOGGER.info("Creating config file for " + modid);
            ...
            SimpleStorageSystems.LOGGER.info("Loading config file for " + modid);
